...
use crate::task::{task_capture, task_change_property, task_change_state, task_get_by_id, task_parent_get, task_set_fields, tasks_ids_get};
...
fn test_set_fields_content_only() {
    ...
    assert_eq!(task.title, "updated task content");
...
fn test_set_fields_state_only() {
    ...
    assert_eq!(task.state, "DONE");
    assert_eq!(task.title, "set state test");
    assert_eq!(task.properties.get("scheduled"), Some(&"2024-06-01".to_string()));
...
fn test_set_fields_multiple_fields() {
    ...
    assert_eq!(task.title, "completely new content");
    assert_eq!(task.state, "IN_PROGRESS");
    assert_eq!(task.properties.get("scheduled"), Some(&"2024-06-01".to_string()));
...
#[test]
fn test_parent_of_child_task() {
    let config = AgendaConfig::new(PathBuf::from("test-vault"));

    let parent = task_parent_get(&config, "test-parent-of.md:2".to_string());
    assert!(parent.is_some(), "child task should have a parent");
    assert_eq!(parent.unwrap().id, "test-parent-of.md:1");

    let grandparent = task_parent_get(&config, "test-parent-of.md:3".to_string());
    assert!(grandparent.is_some(), "grandchild task should have a parent");
    assert_eq!(grandparent.unwrap().id, "test-parent-of.md:2");

    let root_parent = task_parent_get(&config, "test-parent-of.md:4".to_string());
    assert!(root_parent.is_some(), "second child should have a parent");
    assert_eq!(root_parent.unwrap().id, "test-parent-of.md:1");

    let no_parent = task_parent_get(&config, "test-parent-of.md:1".to_string());
    assert!(no_parent.is_none(), "root task should have no parent");
}

#[test]
fn test_parent_of_done_parent() {
    let config = AgendaConfig::new(PathBuf::from("test-vault"));

    let parent = task_parent_get(&config, "test-parent-done.md:2".to_string());
    assert!(parent.is_some(), "child of DONE parent should find parent");
    assert_eq!(parent.unwrap().id, "test-parent-done.md:1");

    let grandparent = task_parent_get(&config, "test-parent-done.md:3".to_string());
    assert!(grandparent.is_some(), "grandchild should find parent even if grandparent is DONE");
    assert_eq!(grandparent.unwrap().id, "test-parent-done.md:2");
}

#[test]
fn test_parent_of_ancestor_dedup() {
    let config = AgendaConfig::new(PathBuf::from("test-vault"));

    let parent = task_parent_get(&config, "test-ancestor-dedup.md:2".to_string());
    assert!(parent.is_some(), "child of DONE parent should find parent");
    assert_eq!(parent.unwrap().id, "test-ancestor-dedup.md:1");

    let parent_of_child = task_parent_get(&config, "test-ancestor-dedup.md:5".to_string());
    assert!(parent_of_child.is_some(), "child of DONE sibling should find DONE parent");
    assert_eq!(parent_of_child.unwrap().id, "test-ancestor-dedup.md:4");
}
